...
OpenYM2413_2::Patch::Patch(int n, const _byte* data)
{
	if (n == 0) {
		AM = (data[0] >> 7) & 1;
		PM = (data[0] >> 6) & 1;
...
void OpenYM2413_2::writeReg(_byte regis, _byte data, const EmuTime &time)
{
...
_byte OpenYM2413_2::read(unsigned address)
{
	return reg[address];
}

void OpenYM2413_2::write(unsigned address, _byte value)
{
...
void OpenYM2413_2::loadState()
{
    ...
    for (i = 0; i < sizeof(reg) / sizeof(reg[0]); i++) {
        ...
        reg[i] = (_byte)saveStateGet(state, tag, 0);
    }
    ...
    for (i = 0; i < sizeof(patches) / sizeof(patches[0]); i++) {
        ...
        sprintf(tag, "KR%d", i);
        patches[i].KR = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "ML%d", i);
        patches[i].ML = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "KL%d", i);
        patches[i].KL = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "TL%d", i);
        patches[i].TL = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "FB%d", i);
        patches[i].FB = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "WF%d", i);
        patches[i].WF = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "AR%d", i);
        patches[i].AR = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "DR%d", i);
        patches[i].DR = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "SL%d", i);
        patches[i].SL = (_byte)saveStateGet(state, tag, 0);
        
        sprintf(tag, "RR%d", i);
        patches[i].RR = (_byte)saveStateGet(state, tag, 0);
    }
