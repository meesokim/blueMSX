...
extern void moonsoundTimerStart(void* ref, int timer, int start, _byte timerRef);
...
class YMF262Slot
{
	public:
		...
		inline void FM_KEYON(_byte key_set);
		inline void FM_KEYOFF(_byte key_clr);
		...
		_byte FB;	// feedback shift value
		int op1_out[2];	// slot1 output for feedback
		_byte CON;	// connection (algorithm) type

		// Envelope Generator 
		_byte eg_type;	// percussive/non-percussive mode 
		...
		_byte eg_sh_rr;	// (release state)
		_byte eg_sel_rr;	// (release state)
...
class YMF262Channel
{
	public:
		YMF262Channel();
		void chan_calc(_byte LFO_AM);
		void chan_calc_ext(_byte LFO_AM);
		...
		//  10 and 13,
		//  11 and 14
		_byte extended;	// set to 1 if this channel forms up a 4op channel with another channel(only used by first of pair of channels, ie 0,1,2 and 9,10,11) 
};
...
class YMF262 : public SoundDevice, public TimerCallback
{
	public:
		...
		void writeReg(int r, _byte v, const EmuTime &time);
		_byte peekReg(int reg);
		_byte readReg(int reg);
		_byte peekStatus();
		_byte readStatus();
		...
		void callback(_byte flag);

        void loadState();
        void saveState();

	private:
		void writeRegForce(int r, _byte v, const EmuTime &time);
		void init_tables(void);
		void setStatus(_byte flag);
		void resetStatus(_byte flag);
		void changeStatusMask(_byte flag);
		void advance_lfo();
		void advance();
		void chan_calc_rhythm(bool noise);
		void set_mul(_byte sl, _byte v);
		void set_ksl_tl(_byte sl, _byte v);
		void set_ar_dr(_byte sl, _byte v);
		void set_sl_rr(_byte sl, _byte v);
		...
		_byte rhythm;			// Rhythm mode
		_byte nts;			// NTS (note select)

		_byte status;			// status flag
		_byte status2;
		_byte statusMask;		// status mask
